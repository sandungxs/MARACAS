...
dusal.gff3 <- read.table(file="dunaliella_salina.gff3",header=F,quote = "#",as.is=T)
dusal.gtf <- dusal.gff3
head(dusal.gff3)

unique(dusal.gff3$V3)

for(i in 1:nrow(dusal.gff3))
{
  current.attributes <- strsplit(dusal.gff3$V9[i],split=";")[[1]]
  if(dusal.gff3$V3[i] == "gene")
  {
    gene.id <- strsplit(current.attributes[2],split="=")[[1]][2]
    dusal.gtf$V9[i] <- paste("gene_id", paste("\"",gene.id,"\";",sep=""))
  } else if(dusal.gff3$V3[i] == "mRNA")
  {
    gene.id <- substr(strsplit(current.attributes[5],split="=")[[1]][2],start = 1,stop = 16)
  ...
  } else if(dusal.gff3$V3[i] == "exon")
  {
    gene.id <- substr(strsplit(current.attributes[1],split="=")[[1]][2],start = 1,stop = 16)
...
write.table(x = dusal.gtf,file = "../../data/dunaliella_salina/annotation/dunaliella_salina.gtf",sep = "\t",row.names = F,col.names = F,quote = F)
...
dusal.genome.data <- read.fasta(file = "../../data/dunaliella_salina/genome/dunaliella_salina.fa",seqtype = "DNA")
chromosome.names <- getName(dusal.genome.data)
chromosome.lengths <- sapply(X=getSequence(dusal.genome.data),FUN = length)
